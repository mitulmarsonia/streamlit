...
class Player:
    def __init__(self, name, speed, maneuver):
        self.name = name
        self.speed = speed
...
class StrategyPlan:
    ...
    def execute(self):
        return f"Executing {self.name}: " + ", ".join(self.steps)

# Define Team A and Team B players
team_a = [
    Player("Player A1", speed=8, maneuver=6),
    Player("Player A2", speed=7, maneuver=7),
    Player("Player A3", speed=6, maneuver=6)
]

team_b = [
    Player("Player B1", speed=5, maneuver=5),  # Weakest player
    Player("Player B2", speed=8, maneuver=8),
    Player("Player B3", speed=9, maneuver=9)
]

# Generate the 5 plans for Team A
def generate_plans():
    plans = []

    # Plan 1: Start with Player B1 and pass to Player A1
    steps = [
        f"Start with {team_b[0].name} (Team B)",
        f"Pass to {team_a[0].name} (Team A)",
        f"Player {team_a[0].name} advances to goal"
    ]
    plans.append(StrategyPlan("Plan:1", steps))

    # Plan 2: Start with Player B1 and pass to Player A2
    steps = [
        f"Start with {team_b[0].name} (Team B)",
        f"Pass to {team_a[1].name} (Team A)",
        f"Player {team_a[1].name} advances to goal"
    ]
    plans.append(StrategyPlan("Plan:2", steps))

    # Plan 3: Start with Player B1, pass to Player A3, then Player A2
    steps = [
        f"Start with {team_b[0].name} (Team B)",
        f"Pass to {team_a[2].name} (Team A)",
        f"Player {team_a[2].name} passes to {team_a[1].name}",
        f"Player {team_a[1].name} advances to goal"
    ]
    plans.append(StrategyPlan("Plan:3", steps))

    # Plan 4: Start with Player B1, pass to Player A2, then Player A1
    steps = [
        f"Start with {team_b[0].name} (Team B)",
        f"Pass to {team_a[1].name} (Team A)",
        f"Player {team_a[1].name} passes to {team_a[0].name}",
        f"Player {team_a[0].name} advances to goal"
    ]
    plans.append(StrategyPlan("Plan:4", steps))

    # Plan 5: Start with Player B1, pass to Player A3, then Player A1
    steps = [
        f"Start with {team_b[0].name} (Team B)",
        f"Pass to {team_a[2].name} (Team A)",
        f"Player {team_a[2].name} passes to {team_a[0].name}",
        f"Player {team_a[0].name} advances to goal"
    ]
    plans.append(StrategyPlan("Plan:5", steps))

    return plans

# Function to display selected strategy
def display_strategy(strategy_name):
    plans = generate_plans()
    strategy = next((plan for plan in plans if plan.name == strategy_name), None)
    if strategy:
        st.write(strategy.execute())
    else:
...
def main():
    ...
    st.subheader(":red[Works Only with Soccer Games]")
    ...
    demo_selected = st.sidebar.radio(label="Select Demo Video", options=["Demo 1", "Demo 2"], horizontal=True)
     ...
     ## Updated Dropdown menu for strategy selection with new strategy names
    strategies = ["Direct Assault", "Flank Attack", "Combination Play", "Quick Transition", "Cross-Field Move"]
    selected_strategy = st.sidebar.selectbox("Select Strategy Plan", options=strategies)
    ...
    model_players.export(format='onnx')
    model_keypoints.export(format='onnx')    
    ...
    with tab1:
        ...
        st.markdown("""
                    **There are two demo videos that are automatically loaded when you start the app, alongside the recommended settings and hyperparameters**
                    ...
                    5. Follow the instructions on the page to pick each team color.
